...
ROOT_PATH = os.getcwd()
config_path = os.path.join(ROOT_PATH, 'doc/config.json')
...
class SharePoint:
    ...
    def get_content_url(self, relative_url,return_list_folder=False):
        self.list_folder = []
...
#         self.ctx.execute_query()
        ...
        for myfolder in self.folders:
            self.list_folder.append(myfolder.properties['ServerRelativeUrl'])
        print("Folder name: {}".format("\n".join(self.list_folder)))    
        if return_list_folder==True:
